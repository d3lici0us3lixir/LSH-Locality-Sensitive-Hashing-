#ifndef CAMELLIA_H
#define CAMELLIA_H

#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <iostream>
#include <cstdlib>
#include <cstdint>
#include <algorithm>
#include <sstream>
#include <iomanip>
#include <vector>
#include <string>
using namespace std;

class CAMELLIA {
public:
	CAMELLIA(uint8_t* PT, uint8_t* KEY);
	void p_function(uint8_t pt[8], uint8_t result[8]);
	void s_function(uint8_t pt[8], uint8_t result[8]);
	void f_function(uint8_t pt[8], uint8_t key[8], uint8_t result[8]);
	void left_shift(uint8_t pt[16], uint8_t result[16], int num);
	void left_shift_for4(uint8_t pt[4], uint8_t result[4], int num);
	void make_keyAB(uint8_t key[16], uint8_t ka[16]);
	void FL(uint8_t pt[8], uint8_t key[8], uint8_t result[8]);
	void FL_inverse(uint8_t pt[8], uint8_t key[8], uint8_t result[8]);
	void key_schedule(uint8_t* key);
	void camelia_round(uint8_t pt[16], uint8_t key[8], uint8_t result[16]);
	string camelia_encrypt(int round);
	string make_return_str(uint8_t* state);
	const uint8_t sbox1[256] =
	{
	   
	};

	//Substitution table 2
	const uint8_t sbox2[256] =
	{
	   
	};

	//Substitution table 3
	const uint8_t sbox3[256] =
	{
	   
	};

	//Substitution table 4
	const uint8_t sbox4[256] =
	{
	   
	};

	uint8_t sigma[6][8] =
	{
		{0xA0, 0x9E, 0x66, 0x7F, 0x3B, 0xCC, 0x90, 0x8B},
		{0xB6, 0x7A, 0xE8, 0x58, 0x4C, 0xAA, 0x73, 0xB2},
		{0xC6, 0xEF, 0x37, 0x2F, 0xE9, 0x4F, 0x82, 0xBE},
		{0x54, 0xFF, 0x53, 0xA5, 0xF1, 0xD3, 0x6F, 0x1C},
		{0x10, 0xE5, 0x27, 0xFA, 0xDE, 0x68, 0x2D, 0x1D},
		{0xB0, 0x56, 0x88, 0xC2, 0xB3, 0xE6, 0xC1, 0xFD}
	};

	uint8_t state[16], state2[16], left[8], right[8], left2[8], right2[8];
	uint8_t pt[16], k[18][8], kw[4][8], kl[4][8], result[16];
};
#endif
